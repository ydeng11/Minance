$(document).ready(function () {
    getAllData("mapping_category/unlinkedCategories").then((data) => {
      ...
      $("#linkCategory").on("click", function (event) {
        ...
        let body = {
          minanceCategory: minanceCategory,
          listRawCategories: listRawCategories
        };
        callApi("http://localhost:8080/1.0/minance/mapping_category/linkCategory", "POST", body)
            .then((response) => {
              if (response.ok) {
      ...
      $("#createMinanceCategory").on("click", function (event) {
        ...
        callApi("http://localhost:8080/1.0/minance/mapping_category/create", "POST", body)
        .then((response) => {
            if (!response.ok) {
        ...
        .then((responseText) => {$("#msgBox").text(responseText)})
      ...
      $("#deleteMinanceCategory").on("click", function (event) {
        ...
        callApi("http://localhost:8080/1.0/minance/mapping_category/delete", "DELETE", body)
            .then((response) => {
                if (!response.ok) {
                    throw new Error('Network response was not ok ' + response.statusText);
                  }
