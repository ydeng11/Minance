...
public class CsvSchemaResource {
  ...
  static String serializeColumns(List<String> columns) {
    return String.join(",", columns);
  }

  static List<String> deserializeColumns(String columns) {
  ...
  public Response createSchema(
      ...
      @FormParam("dataTypes") List<String> dataTypes) {
    ...
    TransactionCsvSchema transactionCsvSchema = new TransactionCsvSchema.Builder()
        ...
        .dataTypes(serializeColumns(dataTypes))
        .build();
  ...
  public Response updateSchema(@PathParam("id") Long id,
    ...
    TransactionCsvSchema updatedSchema = new TransactionCsvSchema.Builder()
        .withBankId(bank.id)
        .withAccountId(account.id)
        .useHeader(useHeader)
        .skipFirstDataRow(skipFirstDataRow)
    ...
    csvSchemaService.updateSchema(updatedSchema);

    return Response.status(Response.Status.OK).entity(updatedSchema).build();
  }
